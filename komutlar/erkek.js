...
if(message.channel.id !== kanal) return message.channel.send(`Bu Komudu Sadece <#${kanal}> Adlı Kanalda Kullanabilirsin ✔️`)
if (!erkekrol) return message.channel.send(`**Sunucuda Erkek Rolü Ayarlanmadığı İçin Komut Kullanılamaz** ❗️ `)

let member = message.mentions.members.first();
if (!member) return message.channel.send(`**Erkek Olarak Kayıt Edeceğin Kullanıcıyı Belirtmelisin** ❗️ `)
let isim = args[1]
if (!isim) return message.channel.send(`**İsmini Belirtmelisin** ❗️ `)
let yaş = args[2]
if (!yaş) return message.channel.send(`**Yaşını Belirtmelisin ❗️ `)
